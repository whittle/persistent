...
class MonadIO m => MonadSqlPersist m where
    askSqlConn :: m SqlBackend

instance (MonadIO m, MonadReader env m, HasPersistBackend env SqlBackend) => MonadSqlPersist m where
    askSqlConn = liftM persistBackend ask
