...
class PersistStoreImpl backend where
    ...
    backendKeyFromValues :: [PersistValue] -> Maybe (BackendKey backend)
...
class (MonadIO m, PersistStoreImpl backend) => PersistStore backend m | m -> backend where
    ...
    -- | Create a new record in the database, returning an automatically created
    -- key (in SQL an auto-increment id).
    insert :: PersistEntity backend val
           => val
           -> m (Key val)
    insert = runWithBackend . insertImpl

    -- | Same as 'insert', but doesn't return a @Key@.
    insert_ :: PersistEntity backend val
            => val -> m ()
    ...
    -- SQL backends currently use the slow default implementation of
    -- @mapM insert@
    insertMany :: PersistEntity backend val
               => [val] -> m [Key val]
    insertMany = runWithBackend . insertManyImpl

    -- | Create a new record in the database using the given key.
    insertKey :: PersistEntity backend val
              => Key val -> val -> m ()
    ...
    -- Unlike 'replace', if a record with the given key does not
    -- exist then a new record will be inserted.
    repsert :: PersistEntity backend val
            => Key val -> val -> m ()
    ...
    -- not exist, so you must use 'insertKey or 'repsert' in
    -- these cases.
    replace :: PersistEntity backend val
            => Key val -> val -> m ()
    replace k = runWithBackend . replaceImpl k

    -- | Delete a specific record by identifier. Does nothing if record does
    -- not exist.
    delete :: PersistEntity backend val
           => Key val -> m ()
...
getJust :: (PersistStore backend m, PersistEntity backend val, Show (Key val)) => Key val -> m val
...
belongsTo ::
  ...
  , PersistEntity backend' ent1
  , PersistEntity backend ent2
